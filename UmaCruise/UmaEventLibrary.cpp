...
bool UmaEventLibrary::LoadUmaMusumeLibrary()
{
	...
	try {
		...
		{	// UmaMusumeLibrary.json
			...
			ATLASSERT(ifs);
			if (!ifs) {
				throw std::runtime_error("Load failed:UmaMusumeLibrary.json");
			}
			json jsonLibrary;
			ifs >> jsonLibrary;
		...
		}
		{
			...
			ATLASSERT(ifs);
			if (!ifs) {
				throw std::runtime_error("Load failed:Common.json");
			}
			json jsonCommon;
			ifs >> jsonCommon;
	...
	if (m_currentIkuseUmaMusume != umaName) {
		...
		if (m_currentIkuseUmaMusume.length()) {
			...
			if (!m_currentIkuseiUmaEvent) {
				ERROR_LOG << L"m_charaEventList Unknown umamusume: " << umaName;
			}
		}
